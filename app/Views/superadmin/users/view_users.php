<main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                    ...
                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Tabel Users</li>
                </ol>
            </nav>
            ...
            <div class="card-body px-0 pb-2">
                <div class="table-responsive p-3" style="max-height: 500px; overflow-y: auto;">
                    <table class="table table-hover table-bordered align-items-center mb-0">
                        ...
                        <tbody>
                            <?php $no = 1 + (10 * ((int) ($pager->getCurrentPage('users') ?? 1) - 1));
                            foreach ($users as $u): ?>
                                <tr>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0"><?= $no++ ?></p>
                                    </td>
                                    <td><p class="text-xs font-weight-bold mb-0"><?= esc($u['username']) ?></p></td>
                                    <td><p class="text-xs font-weight-bold mb-0"><?= esc($u['role']) ?></p></td>
                                    <td class="align-middle">
                                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#modalEditUser<?= $u['id_user'] ?>">Edit</button>
                                        <a href="<?= base_url('superadmin/users/delete/' . $u['id_user']) ?>"
    ...
    <div class="modal fade" id="modalTambahUser" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form action="<?= base_url('superadmin/users/save') ?>" method="post" class="modal-content">
    ...
    <?php foreach ($users as $u): ?>
        <div class="modal fade" id="modalEditUser<?= $u['id_user'] ?>" tabindex="-1">
            <div class="modal-dialog">
                <form action="<?= base_url('superadmin/users/update/' . $u['id_user']) ?>" method="post" class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit User</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3"><label>Username</label><input type="text" name="username" class="form-control" value="<?= esc($u['username']) ?>" required></div>
